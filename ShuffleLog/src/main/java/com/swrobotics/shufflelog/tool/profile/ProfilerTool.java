...
public abstract class ProfilerTool implements Tool {
    ...
    private void drawPieSlice(ImDrawList draw, double angleMin, double angleMax, float centerX, float centerY, double radius, int color) {
    ...
    private void showNode(ProfileNode node, String path, boolean isRoot) {
        if (!isRoot)
            path = path + SEPARATOR + node.getName();
